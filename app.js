    ...
    if (t.Type === 'quiz') {
        ...
        if (pastGuesses !== null) {
            ...
            pastGuessesArr.forEach( guess => {                
                ...
                for(let i=0; i<tArr.length; i++) if(gArr[i]===tArr[i]) { tArr[i]=null; gArr[i]=null; resultArr[i] = "b"}
                for(let i=0; i<gArr.length; i++) if(gArr[i] && tArr.indexOf(gArr[i])!==-1) { tArr[tArr.indexOf(gArr[i])]=null; resultArr[i] = "c" }
...
async function approveTask(row, maxPoints) {
    ...
    if( maxPoints && (pts < 0 || pts > maxPoints )) return alert(`Please assign points between 0 and ${maxPoints}`);
